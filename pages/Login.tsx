...
import { supabase, ClinicalAPI } from '../services/apiService.ts';
    ...
    try {
      if (isRegister) {
        ...
        const newUser: User = {
          ...
          role,
          isApproved: role === UserRole.PATIENT, // Admin/Consultants might need verification
        ...
        await ClinicalAPI.saveProfile(newUser);
        onLogin(newUser);
      } else {
        ...
        // Fetch user profile
        const profile = await ClinicalAPI.getProfile(authData.user.id);
        if (profile) {
          onLogin(profile);
        } else {
        ...
        <div className="p-10 md:p-12 relative z-10">
          <div className="text-center mb-10">
            ...
            <p className="text-slate-500 mt-2 font-medium text-sm leading-relaxed">
              Connect to the Byinks Health Cloud Infrastructure
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-5">
                {isRegister && (
                  <div className="grid grid-cols-2 gap-2 p-1 bg-slate-100 rounded-2xl mb-4">
                    ...
                    <button type="button" onClick={() => setRole(UserRole.CONSULTANT)} className={`py-2.5 text-[10px] font-black rounded-xl transition uppercase tracking-widest ${role === UserRole.CONSULTANT ? 'bg-emerald-600 shadow text-white' : 'text-slate-400'}`}>Consultant</button>
                  </div>
                )}
                {isRegister && (
            ...
            >
              {loading ? 'Processing Cloud Credentials...' : isRegister && step === 1 && role === UserRole.PATIENT ? 'Next: Medical Profile' : isRegister ? 'Initialize Identity' : 'Secure Sign In'}
            </button>
          </form>
