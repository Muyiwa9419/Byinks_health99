...
const socketRef = useRef<Socket | null>(null);
  ...
  const loadChat = async () => {
    try {
      ...
      const serverMessages =
        await ClinicalAPI.getChatMessages(chatId);

      if (mounted) {
        setMessages(serverMessages);

        const remaining =
          getRemainingTime(serverMessages);
      ...
      // Connect Socket.IO
      const socket = ClinicalAPI.connectSocket();
      ...
      socket.on('chat:message', (incomingMsg: Message) => {
        if (!mounted) return;
  ...
  e?: React.FormEvent,
  customMsg?: Partial<Message>
) => {
  if (e) e.preventDefault();

  const text =
    customMsg?.text || inputText.trim();

  if (!text || isExpired) return;

  try {
    const savedMessage =
      await ClinicalAPI.sendChatMessage(
        chatId,
        {
          text,
          isSystem: customMsg?.isSystem,
        }
      );
  ...
  if (messages.length > 0 && !isExpired) {
    if (
      !confirm(
        'Clinical Protocol: Ending this session will permanently clear the dialogue history. Proceed?'
      )
    ) {
      return;
    }
  ...
  try {
    await ClinicalAPI.endChatSession(chatId);
