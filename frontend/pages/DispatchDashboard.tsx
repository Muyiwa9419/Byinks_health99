...
const DispatchDashboard: React.FC<{ user: User }> = ({ user }) => {
  ...
  const deliveryMarkersRef = useRef<Map<string, any>>(
    new Map()
  );
  const routeLinesRef = useRef<Map<string, any>>(
    new Map()
  );
  ...
  // =========================================================
  // LOAD DELIVERIES FROM BACKEND + TRACK ONLINE STATE
  // =========================================================
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        console.log(
          '[DISPATCH DASHBOARD] Loading deliveries for dispatcher:',
          user.id
        );
        ...
        console.log(
          '[DISPATCH DASHBOARD] BACKEND DELIVERIES:',
          backendDeliveries
        );
        ...
        console.log(
          '[DISPATCH DASHBOARD] MY ACTIVE DELIVERIES:',
          activeDeliveries
        );
      ...
      // Read current online state from the local session
      try {
        ...
        if (
          ...
          setIsOnline(
            session.isOnline ?? user.isOnline ?? true
          );
        }
      } catch (error) {
        console.error(
          '[DISPATCH DASHBOARD] Session read error:',
          error
        );
      }
    };
    ...
    // Refresh backend assignments every 5 seconds.
    // This makes the dashboard detect a new pharmacy
    // assignment even if Socket.IO is unavailable.
  ...
  useEffect(() => {
    ...
    const trackLocation = () => {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          console.error(
            '[DISPATCH DASHBOARD] Geo tracking failed:',
            err
          );
        },
        {
  ...
  useEffect(() => {
    ...
    // -------------------------------------------------------
    // Remove route lines for completed/removed deliveries
    // -------------------------------------------------------
    ...
    // -------------------------------------------------------
    // Add/update delivery markers and route lines
    // -------------------------------------------------------
    ...
    async () => {
      ...
      try {
        ...
        try {
          ...
          console.error(
            '[DISPATCH DASHBOARD] Failed to update local session:',
            error
          );
        }
      } catch (error) {
        console.error(
          '[DISPATCH DASHBOARD] ONLINE STATUS UPDATE ERROR:',
          error
        );
    ...
    async (
      ...
      try {
        ...
        try {
          ...
          console.error(
            '[DISPATCH DASHBOARD] NOTIFICATION ERROR:',
            notificationError
          );
        }
      } catch (error) {
