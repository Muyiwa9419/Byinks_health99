  ...
  const fetchData = async () => {
    try {
      ...
      if (!consultantsResponse.ok) {
        const errorData = await consultantsResponse
          .json()
          .catch(() => ({}));

        throw new Error(
          errorData.error || 'Failed to load consultants'
        );
      }
...
const openChat = (thread: ChatThread) => {
  ...
  if (!consultant) {
    alert("Consultant information could not be found.");
    return;
  }
  ...
  const consultant = consultants.find(
    (c) => c.id === bookingForm.consultantId
  );
...
console.log("BOOKING FORM:", bookingForm);
  ...
  if (!consultant) {
    alert("Selected consultant was not found.");
    return;
  }
  ...
  notes: bookingForm.notes,
    ...
    if (!response.ok) {
      throw new Error(
        data.message || data.error || "Failed to book appointment"
      );
    }
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-gradient-to-br from-emerald-800 via-emerald-600 to-teal-900 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden group">
            <div className="relative z-10">
              ...
              <p className="text-emerald-100 font-medium max-w-md">Your health status is being monitored in real-time by the ByinksHealth core.</p>
              <button 
                 ...
                 return (
                   ...
                   >
                     <div className="flex items-center space-x-6">
                       ...
                       <div>
                         <p className="font-black text-slate-900">{consultant.name}</p>
  ...
  try {
    ...
    const availability =
      await ClinicalAPI.getAvailability(consultantId);
    ...
    console.error(
      'Failed to load consultant availability:',
      error
    );
    ...
    setLoadingAvailability(false);
                  ...
                  {consultants.map(c => <option key={c.id} value={c.id}>
  {c.name} ({c.specialty})
</option>)}
                </select>
  ...
  consultantAvailability && (
    <div className="mt-4 p-5 bg-emerald-50 border border-emerald-100 rounded-2xl">
      ...
      <div className="flex flex-wrap gap-2">
        {Object.entries(
          consultantAvailability.blockedSlots || {}
